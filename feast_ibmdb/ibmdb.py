...
from feast_ibmdb.ibmdb_source import IBMdbSource
from feast_ibmdb.ibmdb_type_map import ibmdb_to_pa_value_type, pa_to_ibmdb_value_type
...
try:
    ...
    raise FeastExtrasDependencyImportError("ibmdb", str(e))


class IBMdbOfflineStoreConfig(FeastConfigBaseModel):
    """ Offline store config for IBMdb """

    type: StrictStr = "ibmdb"
    """ Offline store type selector """

    host: StrictStr
    """ Host name of the IBMdbServer2 """

    port: StrictInt = 10000
    """ Port number of IBMdbServer2 (default is 10000) """

    database: StrictStr = "default"
    """ Default database when obtaining new cursors """

    #timeout: Optional[StrictInt] = None
    """ Connection timeout in seconds when communicating with IBMdbServer2 """

    #ibmdb_conf: Optional[Dict[str, str]] = None
    """ Configuration overlay for the IBMdbServer2 session """

    entity_uploading_chunk_size: StrictInt = 10000
    """ The chunk size of multiple insert when uploading entity_df to IBMdb,
        tuning this may affect the performance of get_historical_features """

    use_ssl: StrictBool = False
    """ Use SSL when connecting to IBMdbServer2 """
    ...
    def get_connect_str(self):
        """
        Creates a connection string structured as below. 
        conn_str = 'database=<database>;hostname=<host>;port=<port>;protocol=<;uid=<username>;pwd=<password>'

        """
        conn_str = "database={};hostname={};port={};protocol={};uid={};pwd={}".format( 
...
class IBMdbConnection:
    def __init__(self, store_config: IBMdbOfflineStoreConfig):
        assert isinstance(store_config, IBMdbOfflineStoreConfig)
        self._store_config = store_config
        self._real_conn = Connection(connect(
            store_config.get_connect_str()
        ))

    @property
    def real_conn(self) -> IBM_DB_APIConnection:
        return self._real_conn

    def close(self) -> None:
        self.real_conn.close()

    def cursor(self) -> IBM_DB_APICursor:
...
class IBMdbOfflineStore(OfflineStore):
    @staticmethod
    def pull_latest_from_table_or_query(
        ...
        assert isinstance(config.offline_store, IBMdbOfflineStoreConfig)
        assert isinstance(data_source, IBMdbSource)
        ...
        return IBMdbRetrievalJob(conn, query)
    ...
    def get_historical_features(
        ...
        def query_generator() -> Iterator[List[str]]:
            ...
            try:
                ...
                # In order to use `REGEX Column Specification`, need set `IBMdb.support.quoted.identifiers` to None.
                # Can study more here: https://cwiki.apache.org/confluence/display/IBMdb/LanguageManual+Select
                queries = [
                    "SET ibmdb.support.quoted.identifiers=None",
                    "SET ibmdb.exec.temporary.table.storage=memory",
        ...
        return IBMdbRetrievalJob(conn, query_generator)


class IBMdbRetrievalJob(RetrievalJob):
    def __init__(
        self,
    ...
    def to_arrow(self) -> pa.Table:
        with self._queries_generator() as queries:
            with self._conn.cursor() as cursor:
                ...
                schema = pa.schema(
                    [
                        (field[0], ibmdb_to_pa_value_type(field[1]))
                        for field in cursor.description
                    ]
                )
                pa_batches = [
                    IBMdbRetrievalJob._convert_ibmdb_batch_to_arrow_batch(b, schema)
                    for b in batches
                ]
                return pa.Table.from_batches(pa_batches, schema)

    @staticmethod
    def _convert_ibmdb_batch_to_arrow_batch(
        ibmdb_batch: ImpalaCBatch, schema: pa.Schema
    ) -> pa.RecordBatch:
        return pa.record_batch(
            [
                IBMdbRetrievalJob._get_values_from_column(column)
                for column in ibmdb_batch.columns
            ],
            schema,
        )
...
def _upload_entity_df_and_get_entity_schema(
    ...
    if isinstance(entity_df, pd.DataFrame):
        ...
        limited_entity_df = IBMdbRetrievalJob(
...
def _upload_entity_df_by_insert(
    ...
    """Uploads a Pandas DataFrame to IBMdb by data insert"""
    entity_df.reset_index(drop=True, inplace=True)

    pa_table = pa.Table.from_pandas(entity_df)
    ibmdb_schema = []
    for field in pa_table.schema:
        ibmdb_type = pa_to_ibmdb_value_type(str(field.type))
        if not ibmdb_type:
            raise ValueError(f'Not supported type "{field.type}" in entity_df.')
        ibmdb_schema.append((field.name, ibmdb_type))

    with conn.cursor() as cursor:
        # Create IBMdb temporary table according to entity_df schema
        create_entity_table_sql = f"""
            CREATE TABLE {table_name} (
              {', '.join([f'{col_name} {col_type}' for col_name, col_type in ibmdb_schema])}
            )
            STORED AS PARQUET
            """
        ...
        # Upload entity_df to the IBMdb table by multiple rows insert method
        ...
        for batch in pa_batches:
            ...
            for i in range(len(batch)):
                chunk_data.append(
                    [
                        preprocess_value(batch.columns[j][i].as_py(), ibmdb_schema[j][1])
                        for j in range(len(ibmdb_schema))
                    ]
                )
